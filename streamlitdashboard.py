...
def run_kpi_analysis(df_kpi: pd.DataFrame):
    ...
    sns.lineplot(x=tweets_per_month_kpi.index, y=tweets_per_month_kpi.values, marker="o", ax=ax1)
    ax1.grid(True)
    st.pyplot(fig1)

    st.markdown("**Distribution du Score d’Intensité**")
    fig2, ax2 = plt.subplots(figsize=(10, 4))
    sns.histplot(df_kpi["intensity_score"], bins=20, kde=True, ax=ax2)
    st.pyplot(fig2)

    st.markdown("**Nuage de Mots (KPI)**")
...
# Word Cloud
st.subheader("Nuage de mots (données filtrées)")
all_txt = " ".join(df_filtered["cleaned_text"].dropna())
if len(all_txt) > 0:
    wc = WordCloud(width=800, height=400, background_color="white").generate(all_txt)
    figwc, axwc = plt.subplots(figsize=(10,5))
    axwc.imshow(wc, interpolation="bilinear")
    axwc.axis("off")
    st.pyplot(figwc)
else:
    st.info("Aucun texte filtré pour nuage de mots.")
