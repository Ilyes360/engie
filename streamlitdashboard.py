...
def run_kpi_analysis(df_kpi: pd.DataFrame):
    ...
    # Compter le nombre de tweets par année

    tweets_par_annee = df.groupby('year')['id'].count().reset_index()
    ...
    fig3 = px.bar(
        ...
        title="Nombre de tweets par année",

        labels={'year': 'Année', 'id': 'Nombre de tweets'},
    ...
    # Convertir la colonne 'created_at' en format datetime si nécessaire

    df['created_at'] = pd.to_datetime(df['created_at'])

 

    # Filtrer les données pour janvier, février, mars 2024

    df_filtered = df[(df['created_at'].dt.year == 2024) & (df['created_at'].dt.month.isin([1, 2, 3]))]

 

    # Nettoyer les valeurs de la colonne 'tweet_category' (enlever les espaces supplémentaires)

    df_filtered['tweet_category'] = df_filtered['tweet_category'].str.strip()

 

    # Vérifier les catégories uniques pour s'assurer qu'il n'y a pas de valeurs inattendues

    print(df_filtered['tweet_category'].unique())

 

    # Calcul du nombre de tweets par catégorie par mois

    df_filtered['month'] = df_filtered['created_at'].dt.month

    df_category_count = df_filtered.groupby(['month', 'tweet_category']).size().reset_index(name='count')

 

    # Application d'une carte de couleurs manuelle (si nécessaire)
    ...
    color_map = {
        ...
        # Ajoute d'autres catégories et leurs couleurs ici
    ...
    fig5 = px.bar(df_category_count, x='month', y='count', color='tweet_category',

            labels={'month': 'Mois', 'count': 'Nombre de Tweets', 'tweet_category': 'Catégorie'},

            title='Distribution des catégories de tweets (Janvier, Février, Mars 2024)',
    ...
    # Vérification de la distribution des valeurs dans la colonne sentiment

    sentiment_counts = df['sentiment'].value_counts()

 

    # Calcul du pourcentage de chaque sentiment
    ...
    # Optionnel : Visualisation avec Plotly

   

 

    fig6 = px.pie(names=sentiment_percentage.index, values=sentiment_percentage.values,

             title="Répartition des sentiments",

             labels={"names": "Sentiment", "values": "Pourcentage"})
